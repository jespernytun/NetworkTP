...
void afficher_message(char *message, int lg); /* afficher message found in sujet */
...
void client_tcp(int port, char* hostname, int nbmsg, int lgmsg){
  ...
  for (i=0; i<nbmsg; i++) {
    ...
    charmsg++;
    if (charmsg > 'z') charmsg = 'a'; /* Make sure we print characters and not whatever*/
    sleep(1); // avoids us getting one HUGE message on reception
  }
  close(sock);
}
...
void server_tcp(int port, int nbmsg) {
  ...
  servaddr.sin_family = AF_INET;
  servaddr.sin_port = htons(port);
  servaddr.sin_addr.s_addr = htonl(INADDR_ANY); // any NIC

  /* bind our socket to our IP */
  if (bind(sock, (struct sockaddr *)&servaddr, sizeof(servaddr)) < 0) {
    perror("bind");
    exit(1);
  }
  ...
  /* application TCP loops waiting for connections */
  while (1) {
    ...
    /* new client has successfully connected, we fork off the process*/
    printf("New client connected\n");

    pid = fork();
    if (pid < 0) { // if error on fork it returns zero
      perror("fork");
      close(connfd);
      continue; // continue listening, we don't want our program to end this time
    }

    if (pid == 0) {
      /* we have a child process */
      close(sock); /* our child does not need to listen */
      ...
      /* parent closes connfd and listens */
      close(connfd);
